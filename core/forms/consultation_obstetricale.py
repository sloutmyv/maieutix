"""
Formulaires pour les consultations obstétricales
"""
...
from core.models import ConsultationObstetricale, Patient


class ConsultationObstetricaleForm(forms.ModelForm):
    """
    Formulaire pour créer/modifier une consultation obstétricale
    """
    
    class Meta:
        model = ConsultationObstetricale
        fields = [
            'patient',
            'date_consultation',
            'tension_systolique',
        ...
        widgets = {
            ...
            'examen': forms.Textarea(attrs={
                'class': 'mt-1 block w-full px-2 py-1.5 text-sm border border-gray-300 rounded focus:outline-none focus:ring-1 focus:ring-primary focus:border-primary',
                'rows': 4,
                'placeholder': 'Résultats de l\'examen obstétrical...'
            }),
        ...
        help_texts = {
            ...
            'examen': 'Résultats de l\'examen obstétrical',
    ...
    def clean(self):
        ...
        if patient and patient.type_patient != 'femme':
            raise ValidationError({
                'patient': 'Les consultations obstétricales sont réservées aux femmes.'
            })
        
        return cleaned_data


class ConsultationObstetricaleModalForm(ConsultationObstetricaleForm):
    """
...
class ConsultationObstetricaleQuickForm(forms.ModelForm):
    """
