"""
Vues pour les consultations gynécologiques
"""
...
from core.models import Patient, ConsultationGynecologique
...
def patient_consultations(request, patient_id):
    ...
    try:
        ...
        if patient.type_patient != 'femme':
            return render(request, 'core/consultations/consultation_history.html', {
                'consultations': [],
                'patient': patient,
                'error': 'Les consultations gynécologiques sont réservées aux femmes.'
            })
        
        # Récupérer les consultations ordonnées par date décroissante
        consultations = patient.consultations_gynecologiques.select_related('created_by').all()
...
def consultation_modal(request, patient_id):
    """
    ...
    if request.method == 'GET':
        ...
        form = ConsultationGynecologiqueModalForm(patient_id=patient.id)
        return render(request, 'core/consultations/consultation_modal.html', {
            'form': form,
            'patient': patient
        })
        ...
        form = ConsultationGynecologiqueModalForm(request.POST, patient_id=patient.id)
...
def save_consultation(request):
    ...
    try:
        ...
        with transaction.atomic():
            consultation = ConsultationGynecologique(patient=patient)
...
def delete_consultation(request, consultation_id):
    ...
    """
    try:
        consultation = get_object_or_404(ConsultationGynecologique, pk=consultation_id)
...
def consultation_detail(request, consultation_id):
    ...
    try:
        ...
        return render(request, 'core/consultations/consultation_detail_modal.html', {
...
@login_required
@require_http_methods(["GET"])
def consultation_quick_form(request, patient_id):
    """
    ...
    # Vérifier que c'est bien une femme
    if patient.type_patient != 'femme':
        return render(request, 'core/consultations/consultation_inline_form.html', {
            'form': None,
            'patient': patient,
            'error': 'Les consultations gynécologiques sont réservées aux femmes.'
        })
    
    form = ConsultationGynecologiqueQuickForm(patient=patient)
...
@login_required
@require_http_methods(["POST"])
def save_quick_consultation(request, patient_id):
    """
    ...
    form = ConsultationGynecologiqueQuickForm(request.POST, patient=patient)
    ...
    if form.is_valid():
        try:
            with transaction.atomic():
                ...
                consultations = patient.consultations_gynecologiques.select_related('created_by').all()
                response = render(request, 'core/consultations/consultation_history.html', {
                    'consultations': consultations,
                    'patient': patient
                })
